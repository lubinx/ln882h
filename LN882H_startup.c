...
{
    #ifdef CoreDebug    // CMSIS 6.0 patchs
        if (CoreDebug_DHCSR_C_DEBUGEN_Msk & CoreDebug->DHCSR)
            __BKPT(0);
        else
            NVIC_SystemReset();
    #else
        if (DCB_DHCSR_C_DEBUGEN_Msk & DCB->DHCSR)
            __BKPT(0);
        else
            NVIC_SystemReset();
    #endif
...
void StackOverflow_Handler(void *tcb, uint32_t sp, uint32_t sp_base)
{
    (void)tcb;
    (void)sp;
    (void)sp_base;
 ...
 *----------------------------------------------------------------------------*/
__VECTOR_TABLE_ATTRIBUTE void *const __Vectors[] =
{
